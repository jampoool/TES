<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel='stylesheet' href='https://cdn-uicons.flaticon.com/uicons-regular-rounded/css/uicons-regular-rounded.css'>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/flat-ui/2.2.2/css/flat-ui.min.css" integrity="sha512-PvB3Q4vTvWD/9aiiELYI3uebup/4mtou3Mc/uGudC/Zl+C9BdKUkAI+5jORfA+fvLK4DpzC5VyEN7P2kK43hjg==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <script type="text/javascript" src="dist/js/jquery.js"></script>
    <script type="text/javascript" src=""></script>
    <link href= "guidancedashboard.css" rel= "stylesheet">
    <title>Guidance - Dashboard</title>
</head>
<body>
    <body>
        <div class="dashboard">
            <h5>Dashboard</h5>
            <span>Home / Dashboard</span>
        </div>
    <div class="container">
        <div class="row">
            <div class="blue">
                    <div class="card bg-c-blue order-card">
                        <div class="card-block">
                            <span class="m-b-20">Total Question Category</span>
                            <h2 class="text-right"><i class="fi fi-rr-user"></i><span>0</span></h2>
                        </div>
                    </div>
                </div>
            <div class="yellow">
                    <div class="card bg-c-blue order-card">
                        <div class="card-block">
                            <span class="m-b-20">Total Evaluation Form</span>
                            <h2 class="text-right"><i class="fi fi-rr-user"></i><span>0</span></h2>
                        </div>
                    </div>
                </div>
            <div class="green">
                    <div class="card bg-c-blue order-card">
                        <div class="card-block">
                            <span class="m-b-20">Total Evaluation Report</span>
                            <h2 class="text-right"><i class="fi fi-rr-user"></i><span>0</span></h2>
                        </div>
                    </div>
                </div>
        </div>
</body>
</html>